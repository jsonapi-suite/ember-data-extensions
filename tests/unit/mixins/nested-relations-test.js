...
const Article = DS.Model.extend(ModelMixin, NestedRelationsMixin, {
  author: DS.belongsTo("author", { serializeAsAttributeId: true }),
  content: DS.attr("string"),
  tags: DS.hasMany("tags", { serializeAsAttributeId: true })
});
...
moduleForAcceptance("Unit | Mixin | nested-relations", {
  beforeEach() {
    ...
    getOwner(this).register("model:article", Article);
...
const seedArticleWithAuthor = function() {
  store.pushPayload({
    data: {
      type: "articles",
      id: 1,
...
test("it serializes with relationship id attributes correctly", function(assert) {
  seedArticleWithAuthor();
...
test("excluding attributes", function(assert) {
  ...
  assert.notOk(
    json.data.hasOwnProperty("attributes"),
...
test("it serializes async: false relationships correctly", function(assert) {
  ...
  let json = serialize(post, {
    attributes: false,
    relationships: "asyncFalseAuthor"
  });
  let expectedJSON = {
    data: {
      type: "posts",
...
test("array with nesting", function(assert) {
  ...
  let expectedJSON = {
    data: {
      ...
      relationships: {
        tags: {
          data: [{ type: "tags", "temp-id": tag.tempId(), method: "create" }]
        },
        author: {
          data: {
            type: "authors",
